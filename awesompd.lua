...
function awesompd:run()
   ...
   self.widget:add(self.view_widget)
...
function awesompd:wrap_output(text)
   return format('<span font="%s" foreground="%s" background="%s">%s%s%s</span>',
...
function awesompd:set_text(text)
   ...
   --self.text_widget:set_markup(text)
end
...
function awesompd:try_get_local_cover(current_file)
   if self.mpd_config then
      ...
      local request = format("ls '%s' | egrep -e '\\.jpg|\\.png|\\.gif|\\.jpeg'",
                             string.gsub(folder, "'", "'\\''"))
         ...
         if covers_table.n > 1 then
            ...
            local front_cover =
               self.pread('echo "' .. covers ..
                          '" | grep -i -e "cover|front|folder|albumart" | head -n 1', "*line")
            if front_cover then
